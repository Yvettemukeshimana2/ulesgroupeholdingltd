...
import {
  FaCompass, FaFlag, FaGlobe, FaSitemap, FaChartLine,
  FaCogs, FaMicrochip, FaServer, FaUsers, FaChartBar,
  FaPaintBrush, FaLightbulb, FaHandshake, FaStar, FaShieldAlt,
  FaEnvelope, FaLinkedin, FaPhone, FaUser, FaDesktop,
  FaInfoCircle, FaCrown,
} from "react-icons/fa";
...
const values = [
  { Icon: FaLightbulb,  title: "Innovation",    text: "Pushing the frontier of engineering and digital solutions." },
  { Icon: FaHandshake,  title: "Collaboration", text: "Teams built on trust, shared purpose, and mutual growth." },
  { Icon: FaStar,       title: "Excellence",    text: "The highest standard in every project and every interaction." },
  { Icon: FaShieldAlt,  title: "Integrity",     text: "Transparency and honesty with every client and partner." },
];

const departments = [
  { name: "Operations",             Icon: FaCogs,       lead: "COO",                      color: "#b45309", sub: "Project Mgmt · Procurement · Manufacturing · Admin" },
  { name: "Technology",             Icon: FaMicrochip,  lead: "CTO — David Irakiza",      color: "#1a3a6b", sub: "Mechanical Eng · Prototyping · R&D · Testing" },
  { name: "Information Technology", Icon: FaServer,     lead: "CIO — Yvette Mukeshimana", color: "#0e7490", sub: "Software Dev · Systems Admin · Cybersecurity · Support" },
  { name: "Human Resources",        Icon: FaUsers,      lead: "HR Manager",               color: "#065f46", sub: "Recruitment · Performance · Training · Culture" },
  { name: "Finance",                Icon: FaChartBar,   lead: "Finance Manager / CFO",    color: "#7c2d12", sub: "Budgeting · Accounting · Payroll · Tax Compliance" },
  { name: "Creative & Design",      Icon: FaPaintBrush, lead: "Graphic Designer — Yvan",  color: "#6d28d9", sub: "Visual Design · Brand Identity · UI/UX · Marketing" },
];
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative", zIndex: 10, width: "48%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 6% 0 8%", background: `linear-gradient(105deg, ${C.ink} 0%, ${C.ink} 75%, transparent 100%)` }}>
        ...
        <div key={`info-${animKey}`} style={{ animation: "fup 0.5s 0.35s both" }}>
        ...
        <div style={{ marginTop: 40, display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ width: 160, height: 1.5, background: "rgba(255,255,255,0.12)", borderRadius: 2, overflow: "hidden" }}>
            <div key={`bar-${animKey}`} style={{ height: "100%", background: `linear-gradient(to right,${C.gold},${C.goldXL})`, animation: "growBar 5s linear both" }} />
          </div>
      ...
      <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
        <div key={`ph-${animKey}`} style={{ position: "absolute", inset: 0, animation: "slideIn 0.9s cubic-bezier(0.22,1,0.36,1) both" }}>
          ...
          {s.img
            ...
            : <div style={{ width: "100%", height: "100%", background: C.ink2, display: "flex", alignItems: "center", justifyContent: "center" }}><FaUser style={{ fontSize: 120, color: "rgba(255,255,255,0.08)" }} /></div>
          }
        </div>
          ...
          {heroSlides.map((sl, i) => (
            <button key={i} onClick={() => go(i)} title={sl.name}
              ...
              {sl.img
                ...
                : <div style={{ width: "100%", height: "100%", background: C.ink2, display: "flex", alignItems: "center", justifyContent: "center" }}><FaUser style={{ color: "rgba(255,255,255,0.4)", fontSize: 14 }} /></div>
              }
            </button>
          ))}
        </div>
...
const MDFeature = () => {
  ...
  const mdResponsibilities = [
    ...
    { Icon: FaGlobe,     label: "External Representation" },
    { Icon: FaSitemap,   label: "Department Oversight" },
    { Icon: FaChartLine, label: "Company Growth" },
  ];
  return (
        ...
        >
          <div style={{ position: "relative", background: C.navy, overflow: "hidden" }}>
            ...
            <div style={{ position: "absolute", top: 16, left: 16, width: 40, height: 40, borderRadius: "50%", background: C.gold, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 4px 16px ${C.gold}50` }}>
              <FaCrown style={{ fontSize: 15, color: C.white }} />
            </div>
              ...
              {mdResponsibilities.map(({ Icon, label }, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 10, background: C.cream, border: `1px solid ${C.line}` }}>
                  <div style={{ width: 30, height: 30, borderRadius: 8, background: `${C.navy}12`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <Icon style={{ fontSize: 12, color: C.navy }} />
                  </div>
                  <span style={{ fontSize: 12.5, fontWeight: 600, color: C.ink2 }}>{label}</span>
                </div>
              ))}
            </div>

            <div style={{ display: "flex", gap: 8, paddingTop: 8, borderTop: `1px solid ${C.line}` }}>
              {[FaEnvelope, FaLinkedin, FaPhone].map((Icon, i) => (
                <a key={i} href="#"
                  style={{ width: 36, height: 36, borderRadius: 10, border: `1px solid ${C.line}`, display: "flex", alignItems: "center", justifyContent: "center", color: C.slate, textDecoration: "none", fontSize: 13, transition: "all 0.2s ease" }}
...
/* ─────────────────────────────────────────────
   EXEC CARD
───────────────────────────────────────────── */
type ExecIcon = React.ComponentType<{ style?: React.CSSProperties }>;
type ExPerson = { name: string; role: string; img: string; Icon: ExecIcon; color: string; bio: string; responsibilities: string[] };
    ...
    >
      <div style={{ position: "relative", height: 240, background: p.color + "18", overflow: "hidden", flexShrink: 0 }}>
        {p.img
          ...
          : <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, background: p.color + "12" }}>
              <div style={{ width: 70, height: 70, borderRadius: "50%", border: `1.5px dashed ${p.color}60`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <p.Icon style={{ fontSize: 26, color: p.color + "80" }} />
              </div>
        ...
        <div style={{ position: "absolute", top: 12, right: 12, width: 34, height: 34, borderRadius: "50%", background: p.color, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 4px 12px ${p.color}50` }}>
          <p.Icon style={{ fontSize: 13, color: C.white }} />
        </div>
      ...
      <div style={{ padding: "18px 20px 16px", flex: 1, display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ marginTop: "auto", paddingTop: 12, borderTop: `1px solid ${C.line}`, display: "flex", gap: 6 }}>
          {[FaEnvelope, FaLinkedin, FaPhone].map((Icon, i) => (
            <a key={i} href="#"
              style={{ width: 30, height: 30, borderRadius: 8, border: `1px solid ${C.line}`, display: "flex", alignItems: "center", justifyContent: "center", color: C.slate, textDecoration: "none", fontSize: 11, transition: "all 0.2s ease" }}
...
/* ─────────────────────────────────────────────
   SUPPORT CARD
───────────────────────────────────────────── */
type SuppPerson = { name: string; role: string; dept: string; Icon: ExecIcon; color: string; bio: string; tasks: string[] };
      ...
      <div style={{ padding: "24px 24px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
          <div style={{ width: 52, height: 52, borderRadius: 14, background: p.color + "15", border: `1px solid ${p.color}30`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <p.Icon style={{ fontSize: 20, color: p.color }} />
          </div>
          <div>
...
/* ─────────────────────────────────────────────
   SPECIALIST CARD
───────────────────────────────────────────── */
type SpecPerson = { name: string; role: string; img: string; Icon: ExecIcon; color: string; bio: string };
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "2px 10px", background: p.color + "12", border: `1px solid ${p.color}30`, borderRadius: 20, marginBottom: 8 }}>
          <p.Icon style={{ fontSize: 10, color: p.color }} />
        ...
        <div style={{ display: "flex", gap: 6, marginTop: 14 }}>
          {[FaEnvelope, FaLinkedin].map((Icon, i) => (
            <a key={i} href="#"
              style={{ width: 30, height: 30, borderRadius: 8, border: `1px solid ${C.line}`, display: "flex", alignItems: "center", justifyContent: "center", color: C.slate, textDecoration: "none", fontSize: 11, transition: "all 0.2s ease" }}
              onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.background = p.color; el.style.color = C.white; el.style.borderColor = p.color; }}
              onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.background = "transparent"; el.style.color = C.slate; el.style.borderColor = C.line; }}
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ width: 42, height: 42, borderRadius: 12, background: d.color + "15", border: `1px solid ${d.color}30`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <d.Icon style={{ color: d.color, fontSize: 17 }} />
        </div>
        <div>
  ...
  return (
    ...
    >
      ...
      <div style={{ width: 46, height: 46, borderRadius: 13, background: `${C.gold}12`, border: `1px solid ${C.gold}30`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16, transform: hov ? "rotate(-5deg)" : "none", transition: "transform 0.3s ease" }}>
        <v.Icon style={{ color: C.gold, fontSize: 18 }} />
      </div>
...
const TeamPage = () => {
  const execTeam: ExPerson[] = [
    ...
    { name: "David Irakiza Ndayisaba", role: "Chief Technology Officer", img: t2, Icon: FaMicrochip, color: "#1a3a6b",
    ...
    { name: "Yvette Mukeshimana", role: "Chief Information Officer", img: yv, Icon: FaServer, color: "#0e7490",
  ...
  const supportTeam: SuppPerson[] = [
    { name: "HR Manager", role: "Head of Human Resources", dept: "Human Resources", Icon: FaUsers, color: "#065f46",
    ...
    { name: "Finance Manager", role: "CFO / Finance Department", dept: "Finance", Icon: FaChartBar, color: "#7c2d12",
  ...
  const specialists: SpecPerson[] = [
    { name: "Will", role: "IT Specialist", img: t4, Icon: FaDesktop, color: "#0e7490",
      bio: "Builds and maintains robust IT infrastructure, systems administration, and technical support across the organisation." },
    { name: "Yvan", role: "Graphic Designer", img: "", Icon: FaPaintBrush, color: "#6d28d9",
    ...
    <>
      ...
      <div style={{ background: C.cream, fontFamily: "'Sora', sans-serif" }}>
        ...
        <div style={{ background: C.white, borderBottom: `1px solid ${C.line}` }}>
          <div style={{ maxWidth: 1240, margin: "0 auto", padding: "64px 40px" }}>
            ...
            <div style={{ marginTop: 24, display: "flex", alignItems: "flex-start", gap: 12, padding: "16px 20px", background: `${C.gold}08`, border: `1px solid ${C.gold}25`, borderRadius: 12 }}>
              <FaInfoCircle style={{ color: C.gold, fontSize: 15, marginTop: 1, flexShrink: 0 }} />
              <p style={{ margin: 0, fontSize: 13, color: C.slate, lineHeight: 1.7 }}>
                These roles are being actively staffed as the company scales. Interim responsibilities are shared across the executive team.
              </p>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: C.white, borderBottom: `1px solid ${C.line}` }}>
          <div style={{ maxWidth: 1240, margin: "0 auto", padding: "64px 40px" }}>
            ...
            <div className="dept-grid">
              {departments.map((d, i) => <DeptMini key={i} d={d} />)}
            </div>
          </div>
        </div>
        ...
        <div style={{ background: C.navy, padding: "88px 40px", position: "relative", overflow: "hidden" }}>
          {[{ top: -120, right: -120, size: 480 }, { bottom: -80, left: -80, size: 320 }].map(({ size, ...pos }, i) => (
            <div key={i} style={{ position: "absolute", ...(pos as any), width: size, height: size, borderRadius: "50%", border: `1px solid ${C.goldL}10` }} />
          ))}
